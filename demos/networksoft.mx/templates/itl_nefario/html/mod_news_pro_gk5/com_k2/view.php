...
class NSP_GK5_com_k2_View {
	...
	static function readMore($config, $item) {
		...
		if($config['news_content_readmore_pos'] != 'disabled') {
			...
			//
			if($config['news_content_readmore_pos'] == 'after') {
				return '<a class="btn btn-readmore" href="'.$link.'">'.((trim($config['readmore_text']) != '') ? $config['readmore_text'] : JText::_('MOD_NEWS_PRO_GK5_NSP_READMORE')).'</a>';
			} else {
				return '<a class="btn btn-readmore '.$class.'" href="'.$link.'">'.((trim($config['readmore_text']) != '') ? $config['readmore_text'] : JText::_('MOD_NEWS_PRO_GK5_NSP_READMORE')).'</a>';
			}
		} else {
			return '';
		}
